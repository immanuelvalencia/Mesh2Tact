...
def render_float(sim, depth=None, blur=True, stochastic=False, apply_effects=True, shade_cache=None):
    ...
    if shade_cache is not None and not cfg.optics.boundary_enabled and not cfg.optics.uses_lookup and not cfg.optics.uses_spatial_response:
    ...
    use_manual_background = sim.gel_lighting.enabled and renderer._background is None
    ...
    rgb = renderer.render(sim.sampler, -depth, use_gpu=False, as_uint8=False,
                          contact_depth=sim.raw_depth)
    if use_manual_background:
        rgb = sim.gel_lighting.overlay(rgb)
    rgb = sim.gel_lighting.adjust(rgb)
    rgb = renderer.depth_overlay(rgb, depth)
    if cfg.optics.perimeter_shadow_enabled:
        rgb *= renderer.perimeter_shadow(depth, sim.raw_depth)[..., None]
    ...
    if stochastic:
        ...
        effects.read_noise = effects.shot_noise = effects.speckle = effects.texture = effects.hd_texture = 0
    if not blur:
        effects.blur_px = 0
...
def fit_references(settings, references, size=None, fit_background=True,
    ...
    if algorithm in ('spatial_response', 'indentation_response'):
        from .calibration_spatial import fit_spatial
        return fit_spatial(settings, references, size=size, fit_background=fit_background,
            fit_lighting=fit_lighting, fit_blur=fit_blur, max_nfev=max_nfev,
            cancelled=cancelled, progress=progress, fit_directions=fit_directions,
            fit_filters=fit_filters, fit_noise=fit_noise, fit_texture=fit_texture,
            search_starts=search_starts, calibration_seed=calibration_seed,
            indentation_response=algorithm == 'indentation_response')
    ...
    active_optics = SensorConfig.from_dict(settings['sensor']).optics
    if active_optics.uses_lookup:
        raise ValueError('Select analytic lighting (disable the calibration lookup table) before fitting')
    if fit_lighting and active_optics.uses_spatial_response:
    ...
    def images(data, training_only=False):
        output = []
        optical_key = json.dumps((data['sensor']['optics'], data['gel_lighting']), sort_keys=True)
        for ref, sim, depth in zip(references, sims, depths):
            ...
            # Filters operate after optical shading. Reuse that fixed image
            # when blur/tone/noise settings change; one bounded entry per ref.
            if getattr(sim, '_calibration_optical_key', None) != optical_key:
                sim._calibration_optical_rgb = render_float(sim, depth,
                    apply_effects=False, shade_cache=sim._calibration_shade_cache)
                sim._calibration_optical_key = optical_key
            effects = deepcopy(sim.effects)
            effects.read_noise = effects.shot_noise = effects.speckle = effects.texture = effects.hd_texture = 0
            output.append(effects.apply(sim._calibration_optical_rgb,
                                       sim.cfg.gel.size_x, sim.cfg.gel.size_y))
        return output

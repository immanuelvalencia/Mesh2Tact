...
class GeometricSim:
    ...
    def prepare(self):
        ...
        key = (*self.rotation, *self.offset, c.width, c.height, g.size_x, g.size_y)
        if key != self._key:
    ...
    def export(self, directory, processed_mesh_ref=None, outputs=None):
        ...
        image_outputs = [(outputs.tactile, "tactile.png", rgb[:, :, ::-1]),
                         (outputs.clean, "tactile_clean.png", self.clean_rgb[:, :, ::-1]),
                           (outputs.depth_image, "depth.png", GelSightRenderer.depth_colormap(depth, self.cfg.camera.max_depth)[:, :, ::-1]),

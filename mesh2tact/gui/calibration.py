...
class CalibrationPanel(QtWidgets.QWidget):
    def __init__(self, window):
        ...
        self.lighting = QtWidgets.QCheckBox('Fit contact colour response')
        ...
        self.algorithm.addItem('Indentation-aware contact, colour bands and shadows (recommended)', 'indentation_response')
        self.algorithm.addItem('Measured background + spatial contact response (legacy)', 'spatial_response')
        self.algorithm.addItem('Legacy LED model — robust least squares', 'least_squares')
        self.algorithm.addItem('Learned surrogate (experimental)', 'ai_surrogate')
        self.algorithm.setToolTip('Use an empty-pad image and aligned contacts at multiple known indentations and positions. The recommended mode searches metric boundary spread and fits signed RGB contact/boundary responses. Validation images are never used for selection.')
        self.algorithm.currentIndexChanged.connect(self.update_fit_mode)
        self.lighting.toggled.connect(self.update_fit_mode)
        ...
        note = QtWidgets.QLabel('Save an empty-pad reference and contacts at several positions and known indentations. Include inner edges such as a ring when available. Lock geometry before fitting. The recommended mode fits the background, sharp contact gradients and widening colour/shadow bands; it resets old depth overlays and tone controls before optional camera refinement. Review boundary errors at every depth. Validation images never select fitted parameters.')
    ...
    def update_fit_mode(self, *args):
        measured = self.algorithm.currentData() in ('spatial_response', 'indentation_response')
        self.directions.setEnabled(not measured and self.lighting.isChecked())
        self.lighting.setText('Fit contact gradients and indentation-dependent bands' if self.algorithm.currentData() == 'indentation_response' else 'Fit position-dependent contact response' if measured else 'Fit light colour / intensity')
    ...
    def update_report(self):
        ...
        for row in self.result['metrics']:
            ...
            if row.get('after_boundary_mae') is not None:
                lines.append(f"  Boundary MAE: {row['before_boundary_mae']:.4f} → {row['after_boundary_mae']:.4f}")
    ...
    def finished(self):
        ...
        self.run_button.setEnabled(True)
        self.cancel.setEnabled(False)
        verification = next((s for s in reversed(self.result['stages']) if s['stage'] == 'Production verification'), None) if self.result else None
    ...
    def export(self):
        if self.result is None:
            return
        verification = next((stage for stage in reversed(self.result['stages'])

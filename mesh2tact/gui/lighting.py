...
class LightingPanel(QtWidgets.QGroupBox):
    ...
    def __init__(self, show_presets=True):
        ...
        for title, callback in [("Add light", self._add), ("Remove", self._remove)]:
            ...
            self.light_actions.append(button)
        ...
        self.boundary_enabled = QtWidgets.QCheckBox("Indentation-dependent colour and shadow bands")
        self.boundary_enabled.setToolTip("Adds signed shading around outer and inner boundaries. The spread grows with each contact's indentation; exported depth is unchanged.")
        layout.addWidget(self.boundary_enabled)
        self.boundary_width_mm = self._control(layout, "Boundary starting spread", .01, 2, .01, " mm")
        self.boundary_growth = self._control(layout, "Boundary spread per mm indentation", 0, 2, .05, " mm/mm")
        self.boundary_gain = self._control(layout, "Boundary colour / shadow strength", 0, 2, .05, "")
        self.boundary_enabled.toggled.connect(self._edit)
        self.depth_overlay_enabled = QtWidgets.QCheckBox("Depth brightness overlay")
        self.depth_overlay_enabled.setToolTip("Use geometric indentation as a brightness mask. Deeper contact becomes brighter; the empty pad stays unchanged.")
        layout.addWidget(self.depth_overlay_enabled)
        self.depth_overlay_strength = self._control(layout, "Depth overlay strength", 0, 2, .05, "")
        self.depth_overlay_range_mm = self._control(layout, "Depth for full overlay", .01, 20, .05, " mm")
        self.depth_overlay_falloff = self._control(layout, "Depth mask falloff", .2, 5, .05, "")
        self.depth_overlay_falloff.setToolTip("1 follows depth directly. Higher values concentrate the effect in the deepest regions; lower values spread it across shallow contact.")
        self.depth_overlay_invert = QtWidgets.QCheckBox("Invert overlay: darken deeper contact")
        layout.addWidget(self.depth_overlay_invert)
        self.depth_overlay_enabled.toggled.connect(self._edit)
        self.depth_overlay_invert.toggled.connect(self._edit)
        self.shadow_panel = QtWidgets.QGroupBox("Shadows")
        shadow_layout = QtWidgets.QVBoxLayout(self.shadow_panel)
        layout.addWidget(self.shadow_panel)
        self.perimeter_shadow_enabled = QtWidgets.QCheckBox("Soft perimeter shadow")
        self.perimeter_shadow_enabled.setToolTip("Shades only outward from contact. Contact interiors and enclosed holes stay unchanged. Width grows with indentation.")
        shadow_layout.addWidget(self.perimeter_shadow_enabled)
        self.perimeter_shadow_opacity = self._control(shadow_layout, "Perimeter opacity (0–1)", 0, 1, .01, "")
        self.perimeter_shadow_width_mm = self._control(shadow_layout, "Perimeter starting width", .01, 2, .01, " mm")
        self.perimeter_shadow_growth = self._control(shadow_layout, "Width growth per mm of indentation", 0, 2, .05, " mm/mm")
        self.perimeter_shadow_softness = self._control(shadow_layout, "Perimeter softness", .2, 2, .05, "")
        self.perimeter_shadow_enabled.toggled.connect(self._edit)
        ...
        self.shadow_strength = self._control(shadow_layout, "Directional shadows (0 = off)", 0, 1, .05, "")
        ...
        self.shadow_azimuth = self._control(shadow_layout, "Shadow light direction", -360, 360, 1, "°")
        self.shadow_elevation = self._control(shadow_layout, "Shadow light elevation", 5, 85, 1, "°")
    ...
    def set_optics(self, optics):
        ...
        self.response_notice.setText('Measured contact response is active. Refit with indentation-aware calibration to learn contact gradients and surrounding colour/shadow bands. LED colour/direction controls do not affect fitted responses. A measured background replaces manual pad glows; exposure and camera effects still apply.'
        ...
        if optics.contact_depth_response is not None:
            self.response_notice.setText('Measured contact and flat-face colour responses are active. LED colour/direction controls do not affect these fitted responses. Exposure, boundary strength, and camera effects remain available. Use the reference-lighting fit to recalibrate this preset with its sphere and ring collection.')
        ...
        self.boundary_enabled.setChecked(optics.boundary_enabled)
        for name in ('boundary_width_mm', 'boundary_growth', 'boundary_gain'):
            getattr(self, name).setValue(getattr(optics, name))
        self.depth_overlay_enabled.setChecked(optics.depth_overlay_enabled)
        self.depth_overlay_invert.setChecked(optics.depth_overlay_invert)
        for name in ('depth_overlay_strength', 'depth_overlay_range_mm', 'depth_overlay_falloff'):
            getattr(self, name).setValue(getattr(optics, name))
        self.perimeter_shadow_enabled.setChecked(optics.perimeter_shadow_enabled)
        for name in ('perimeter_shadow_opacity', 'perimeter_shadow_width_mm', 'perimeter_shadow_growth', 'perimeter_shadow_softness'):
            getattr(self, name).setValue(getattr(optics, name))
        ...
        self._loading = False
        self._select()
        self.sync_active_controls()

    def sync_active_controls(self):
        o = self.optics
        measured = o.uses_spatial_response or o.uses_lookup
        boundary_fallback = o.boundary_enabled and o.boundary_response is None and not o.uses_spatial_response
        enabled = not measured or boundary_fallback
        for widget in (self.side, self.selector, self.color_button, self.light_side,
                       self.ambient_button, *self.light_actions):
            widget.setEnabled(enabled)
        for widget in (self.side_distance, self.side_falloff, self.azimuth, self.elevation,
                       self.intensity, self.diffuse, self.specular, self.shininess):
            widget.setEnabled(enabled)
            widget.linked_slider.setEnabled(enabled)
            widget.caption.setEnabled(enabled)
        self.reference.setEnabled(not o.uses_measured_background and not o.uses_lookup)
        self.response_notice.setVisible(measured or o.uses_measured_background)
        self.response_notice.setText('Measured lighting is active. Choose the surface model and edit fitted coefficients in Background and measured lighting. '+('LED controls supply the boundary fallback only.' if boundary_fallback and measured else 'LED colour, direction and reflection controls are inactive.' if measured else 'LED controls remain active; the measured image supplies the background.'))
    ...
    def _edit(self, *_):
        ...
        self.optics.boundary_enabled = self.boundary_enabled.isChecked()
        for name in ('boundary_width_mm', 'boundary_growth', 'boundary_gain'):
            setattr(self.optics, name, getattr(self, name).value())
        self.optics.depth_overlay_enabled = self.depth_overlay_enabled.isChecked()
        self.optics.depth_overlay_invert = self.depth_overlay_invert.isChecked()
        for name in ('depth_overlay_strength', 'depth_overlay_range_mm', 'depth_overlay_falloff'):
            setattr(self.optics, name, getattr(self, name).value())
        self.optics.perimeter_shadow_enabled = self.perimeter_shadow_enabled.isChecked()
        for name in ('perimeter_shadow_opacity', 'perimeter_shadow_width_mm', 'perimeter_shadow_growth', 'perimeter_shadow_softness'):
            setattr(self.optics, name, getattr(self, name).value())

...
def dataset_modes(root: Path) -> list[tuple[str, str]]:
    ...
    if all((root / split).is_dir() for split in ("train", "val", "test")):
        ...
        try:
            gelsight = json.loads(summary.read_text(encoding='utf-8')).get('dataset_kind') == 'gelsight'
        except (OSError, ValueError, AttributeError):
            gelsight = False
        modes = [("This dataset", "single"), ("GelSight  -  raw RGB dataset", "gelsight")]
        return list(reversed(modes)) if gelsight else modes
    available = [name for name in IMAGE_TYPES if (root / name / "train").is_dir()]
    if available:
        return [("All available image types (separate models)", "all"),
                *((name.title(), name) for name in available)]
    return []
...
def inspect_dataset(root: Path, mode: str) -> dict:
    ...
    elif mode in (*IMAGE_TYPES, "gelsight"):
    ...
    if mode == "all":
        ...
        if any(value != first for value in counts.values()):
...
class TrainingWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.dataset_edit.setPlaceholderText("Prepared dataset folder (GelSight or aligned image branches)")
    ...
    def browse_dataset(self):
        ...
        if self.dataset_mode.count() == 0:
            QtWidgets.QMessageBox.warning(self, "Not prepared", "Run preprocess.py first, then choose the exported train/val/test dataset.")
    ...
    def command_arguments(self):
        ...
        args = [str(Path(__file__).resolve().parents[2] / "train.py"), "--dataset-dir", str(folder),

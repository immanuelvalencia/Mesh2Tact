...
reports follow Touchlab-VTS/train_suite.py. Checkpoints are directly readable
by Mesh2Tact's Predict tab.
"""
...
IMAGE_TYPES = ("tactile", "default", "clean")
...
def parse_args(argv=None):
    ...
    dataset_mode.add_argument("--image-type", choices=IMAGE_TYPES,
                              help="Select one branch when dataset-dir contains clean/tactile/default")
...
def resolve_datasets(root: Path, image_type: str | None, all_image_types: bool) -> list[tuple[str | None, Path]]:
    ...
    if all_image_types:
        if len(available) != len(IMAGE_TYPES):
            raise ValueError("--all-image-types requires clean, tactile, and default datasets.")
        return [(name, root / name) for name in IMAGE_TYPES]
    raise ValueError("Choose --image-type clean|tactile|default or --all-image-types for this dataset root.")
...
def save_comparison(output_dir: Path, completed: list[dict]) -> Path:
    """Save one row per architecture/weight with all three test results side by side."""
    ...
    fields = ["architecture", "weights", "seed", "complete_triplet"]
    ...
    for (architecture, weights, seed), variants in sorted(grouped.items()):
        row = {"architecture": architecture, "weights": weights, "seed": seed,
               "complete_triplet": all(name in variants for name in IMAGE_TYPES)}
        for variant in IMAGE_TYPES:
